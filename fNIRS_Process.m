...
if ~isa(user_vars,'struct')
    ...
    user_vars.stim_names={'nback_0a','nback_1a','nback_0b','nback_2a'};
...
mod_stimTable = stimTableMapper(nirs.createStimulusTable(data_raws(1)),...
...
for c=2:len-1
    stim_table.(c).name   = new_names{c};
    stim_table.(c).onset  = new_onsets(c);
    stim_table.(c).dur    = new_durs(c);
end
end
end