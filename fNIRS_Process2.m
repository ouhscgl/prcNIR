function [GroupStats, demograph, stimulus] ...
    = fNIRS_Process2(load_path, nirstoolbox_path, user_vars)
% fNIRS_Process - Main processing core for fNIRS pipeline with batch stimulus support.
% 
% Options: 
...
%
% NEW BATCH STIMULUS SUPPORT:
%     user_vars.stim_onset can now be:
%     - Single array: [onset1, onset2, ...] (applied to all datasets)
%     - Matrix: Each row contains onsets for one dataset
%     - Cell array: Each cell contains onsets for one dataset
%     
%     user_vars.stim_dur can follow the same pattern as stim_onset
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% NEW: Validate and prepare batch stimulus parameters
[batch_stim_onsets, batch_stim_durs] = prepareBatchStimulusParams(...
    user_vars.stim_onset, user_vars.stim_dur, length(data_raws), user_vars.stim_names);

% Process each dataset with its specific stimulus parameters
for dataset_idx = 1:length(data_raws)
    fprintf('Processing dataset %d/%d with custom stimulus parameters...\n', ...
            dataset_idx, length(data_raws));
    
    % Get stimulus parameters for this specific dataset
    current_stim_onset = batch_stim_onsets{dataset_idx};
    current_stim_dur = batch_stim_durs{dataset_idx};
    
    % Stimulus correction for current dataset ____________________________
    %-- Change stimulus data ( nirs.getStimNames(data_raws) );
    job = nirs.modules.ChangeStimulusInfo();
    [data_raws(dataset_idx), mod_stimTable, rename_mapping] = createRobustStimMapping(...
       data_raws(dataset_idx), user_vars.stim_names, current_stim_onset, current_stim_dur);
    job.ChangeTable = mod_stimTable;
    %-- Rename stimuli
    job = nirs.modules.RenameStims(job);
    job.listOfChanges = rename_mapping;
    data_raws(dataset_idx) = job.run(data_raws(dataset_idx));
    % ____________________________________________________________________
end

% Continue with standard processing pipeline _____________________________
...
% NEW: Batch stimulus preparation function ________________________________
function [batch_stim_onsets, batch_stim_durs] = prepareBatchStimulusParams(...
    stim_onset, stim_dur, num_datasets, stim_names)
    
    fprintf('Preparing batch stimulus parameters for %d datasets...\n', num_datasets);
    
    % Initialize output cell arrays
    batch_stim_onsets = cell(num_datasets, 1);
    batch_stim_durs = cell(num_datasets, 1);
    
    % Process stimulus onsets
    if iscell(stim_onset)
        % Cell array format: each cell contains onsets for one dataset
        if length(stim_onset) == num_datasets
            batch_stim_onsets = stim_onset;
            fprintf('Using cell array format for stimulus onsets (one cell per dataset)\n');
        elseif length(stim_onset) == 1
            % Single cell containing onsets to be applied to all datasets
            for i = 1:num_datasets
                batch_stim_onsets{i} = stim_onset{1};
            end
            fprintf('Replicating single cell of stimulus onsets across all datasets\n');
        else
            error('Cell array length (%d) does not match number of datasets (%d)', ...
                  length(stim_onset), num_datasets);
        end
    elseif ismatrix(stim_onset) && size(stim_onset, 1) > 1
        % Matrix format: each row contains onsets for one dataset
        if size(stim_onset, 1) == num_datasets
            for i = 1:num_datasets
                batch_stim_onsets{i} = stim_onset(i, :);
                % Remove NaN values if present
                batch_stim_onsets{i} = batch_stim_onsets{i}(~isnan(batch_stim_onsets{i}));
            end
            fprintf('Using matrix format for stimulus onsets (one row per dataset)\n');
        else
            error('Matrix rows (%d) does not match number of datasets (%d)', ...
                  size(stim_onset, 1), num_datasets);
        end
    else
        % Single array or scalar: replicate across all datasets
        if isscalar(stim_onset) && isnan(stim_onset)
            % NaN means use original onsets
            for i = 1:num_datasets
                batch_stim_onsets{i} = NaN;
            end
            fprintf('Using original stimulus onsets for all datasets (NaN provided)\n');
        else
            % Replicate the same onsets across all datasets
            for i = 1:num_datasets
                batch_stim_onsets{i} = stim_onset;
            end
            fprintf('Replicating single array of stimulus onsets across all datasets\n');
        end
    end
    
    % Process stimulus durations (similar logic)
    if iscell(stim_dur)
        % Cell array format
        if length(stim_dur) == num_datasets
            batch_stim_durs = stim_dur;
            fprintf('Using cell array format for stimulus durations\n');
        elseif length(stim_dur) == 1
            for i = 1:num_datasets
                batch_stim_durs{i} = stim_dur{1};
            end
            fprintf('Replicating single cell of stimulus durations across all datasets\n');
        else
            error('Cell array length (%d) does not match number of datasets (%d)', ...
                  length(stim_dur), num_datasets);
        end
    elseif ismatrix(stim_dur) && size(stim_dur, 1) > 1
        % Matrix format
        if size(stim_dur, 1) == num_datasets
            for i = 1:num_datasets
                batch_stim_durs{i} = stim_dur(i, :);
                batch_stim_durs{i} = batch_stim_durs{i}(~isnan(batch_stim_durs{i}));
            end
            fprintf('Using matrix format for stimulus durations\n');
        else
            error('Matrix rows (%d) does not match number of datasets (%d)', ...
                  size(stim_dur, 1), num_datasets);
        end
    else
        % Single array or scalar
        if isscalar(stim_dur) && isnan(stim_dur)
            for i = 1:num_datasets
                batch_stim_durs{i} = NaN;
            end
            fprintf('Using original stimulus durations for all datasets (NaN provided)\n');
        else
            for i = 1:num_datasets
                batch_stim_durs{i} = stim_dur;
            end
            fprintf('Replicating stimulus durations across all datasets\n');
        end
    end
    
    % Validation: Check that each dataset has consistent stimulus parameters
    for i = 1:num_datasets
        if ~isnan(batch_stim_onsets{i}(1)) && length(batch_stim_onsets{i}) ~= length(stim_names)
            warning('Dataset %d: Number of stimulus onsets (%d) does not match number of stimulus names (%d)', ...
                    i, length(batch_stim_onsets{i}), length(stim_names));
        end
        
        if ~isnan(batch_stim_durs{i}(1)) && length(batch_stim_durs{i}) ~= length(stim_names)
            warning('Dataset %d: Number of stimulus durations (%d) does not match number of stimulus names (%d)', ...
                    i, length(batch_stim_durs{i}), length(stim_names));
        end
    end
    
    fprintf('Batch stimulus parameters prepared successfully!\n');
    
    % Display summary
    fprintf('\nBatch Stimulus Summary:\n');
    fprintf('Number of datasets: %d\n', num_datasets);
    fprintf('Number of stimulus conditions: %d\n', length(stim_names));
    for i = 1:min(3, num_datasets)  % Show first 3 datasets as example
        fprintf('Dataset %d onsets: ', i);
        if isnan(batch_stim_onsets{i}(1))
            fprintf('[using original]\n');
        else
            fprintf('[%s]\n', sprintf('%.1f ', batch_stim_onsets{i}));
        end
    end
    if num_datasets > 3
        fprintf('... (and %d more datasets)\n', num_datasets - 3);
    end
end
...
function [stim_table] = stimTableMapper(stim_table, ...
    ...
    % NEW FEATURE: Check if we need to recreate markers from scratch
    recreate_markers = false;
    
    % Check if stimulus onset is provided and not NaN
    if ~(isscalar(stim_onset) && isnan(stim_onset))
        % Count total existing markers across all stimulus channels
        total_existing_markers = 0;
        for i = 1:length(data_raws)
            stim_keys = data_raws(i).stimulus.keys;
            for j = 1:length(stim_keys)
                key = stim_keys{j};
                stim = data_raws(i).stimulus(key);
                total_existing_markers = total_existing_markers + stim.count;
            end
            break; % Only check first subject for marker count
        end
        
        % Determine expected number of markers
        expected_markers = length(stim_onset);
        if isscalar(stim_onset)
            expected_markers = length(stim_names);
        end
        
        % Check if we have a mismatch
        if total_existing_markers ~= expected_markers
            disp(['Mismatch detected: Expected ', num2str(expected_markers), ...
                  ' markers but found ', num2str(total_existing_markers), ' existing markers.']);
            disp('Recreating stimulus markers from scratch using user variables...');
            recreate_markers = true;
        end
    end
    
    % RECREATE MARKERS FROM SCRATCH if needed
    if recreate_markers
        % Validate inputs for marker recreation
        if isscalar(stim_onset) && ~isnan(stim_onset)
            % If single onset provided, create equally spaced markers
            warning('Single onset value provided. Creating equally spaced markers.');
            spacing = 100; % Default spacing in seconds, adjust as needed
            onset_values = stim_onset + (0:(length(stim_names)-1)) * spacing;
        else
            onset_values = stim_onset;
        end
        
        % Ensure we have enough onset values
        if length(onset_values) < length(stim_names)
            error('Not enough onset values provided for stimulus recreation');
        end
        
        % Handle duration values
        if isscalar(stim_dur) && ~isnan(stim_dur)
            dur_values = repmat(stim_dur, 1, length(stim_names));
        else
            dur_values = stim_dur;
        end
        
        % Ensure we have enough duration values
        if length(dur_values) < length(stim_names)
            warning('Not enough duration values. Using last duration for remaining stimuli.');
            dur_values = [dur_values, repmat(dur_values(end), 1, length(stim_names) - length(dur_values))];
        end
        
        % Recreate stimulus markers for each subject
        for i = 1:length(data_raws)
            % Clear existing stimulus data
            stim_keys = data_raws(i).stimulus.keys;
            for j = 1:length(stim_keys)
                data_raws(i).stimulus(stim_keys{j}) = [];
            end
            
            % Create new stimulus markers
            for j = 1:length(stim_names)
                stim_key = sprintf('recreated_stim_%d', j);
                
                % Create new stimulus event
                new_stim = nirs.design.StimulusEvents();
                new_stim.name = stim_key;
                new_stim.onset = onset_values(j);
                new_stim.dur = dur_values(j);
                new_stim.amp = 1; % Default amplitude
                
                % Add to stimulus collection
                data_raws(i).stimulus(stim_key) = new_stim;
                
                disp(['Created stimulus marker: ', stim_key, ...
                      ' at onset ', num2str(onset_values(j)), ...
                      ' with duration ', num2str(dur_values(j))]);
            end
        end
        
        % Update available stimuli list after recreation
        available_stims = cell(1, length(stim_names));
        for j = 1:length(stim_names)
            available_stims{j} = sprintf('recreated_stim_%d', j);
        end
        
        disp('Successfully recreated stimulus markers from user variables.');
        
        % Skip the rest of the restructuring since we've created fresh markers
        events_created = false;
    else
        % EXISTING CODE - Restructure stimuli if we have fewer stimulus channels than expected condition names
        events_created = false;
        for i = 1:length(data_raws)
            stim_keys = data_raws(i).stimulus.keys;
    ...
    if is_group_analysis && ~recreate_markers
        ...
        % Standard case for single-subject processing or after recreation
        if ~recreate_markers
            available_stims = nirs.getStimNames(data_raws);
        end
        disp('Available stimuli in raw data (after processing):');
        disp(available_stims);
    end
    ...
    elseif length(stim_names) ~= length(available_stims)
        ...
        % For recreated markers, we want exact mapping
        if recreate_markers
        ...
        % === EXISTING GROUP ANALYSIS CODE ===
        elseif is_group_analysis
    ...
    %-- Handle NaN in onsets/durations (only if not recreated)
    use_original_onsets = false;
    if ~recreate_markers && isscalar(stim_onset) && isnan(stim_onset)
        use_original_onsets = true;
        disp('Using original stimulus onsets (NaN provided)');
    end
    
    use_original_durs = false;
    if ~recreate_markers && isscalar(stim_dur) && isnan(stim_dur)
        use_original_durs = true;
        disp('Using original stimulus durations (NaN provided)');
    end

    %-- Create stimulus table and mapping
    try
        ...
        % For recreated markers, we use new values directly
        if recreate_markers
            mod_stimTable = orig_stim_table; % The table now contains our recreated markers
            
            % Create mapping for recreated markers
            rename_mapping = cell(length(available_stims), 2);
            for i = 1:length(available_stims)
                rename_mapping{i, 1} = available_stims{i};
                if i <= length(stim_names)
                    rename_mapping{i, 2} = stim_names{i};
                else
                    rename_mapping{i, 2} = available_stims{i};
                end
            end
            
            disp('Created mapping for recreated stimulus markers:');
            disp(rename_mapping);
        else
            % Existing logic for non-recreated markers
            if use_original_names && use_original_onsets && use_original_durs
                ...
                % Call the mapper with appropriate values
            ...
            if use_original_names
                ...
                % For event-based stimuli, preserve numeric order
        ...
        disp('Final stimulus renaming mapping:');
        disp(rename_mapping);
